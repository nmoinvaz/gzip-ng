...
execute_process(COMMAND ${EXE} -m -c ${WORKDIR}/t2/same.txt OUTPUT_FILE ${WORKDIR}/m2.gz RESULT_VARIABLE rc2)
execute_process(COMMAND ${CMAKE_COMMAND} -E compare_files ${WORKDIR}/m1.gz ${WORKDIR}/m2.gz
                RESULT_VARIABLE rc3)
file(READ ${WORKDIR}/m1.gz MHEAD HEX LIMIT 20)
string(SUBSTRING "${MHEAD}" 8 8 MTIME_FIELD)
string(FIND "${MHEAD}" "73616d652e747874" mname_at)
if(NOT rc EQUAL 0 OR NOT rc2 EQUAL 0 OR NOT rc3 EQUAL 0)
    message(FATAL_ERROR "-m output was not reproducible")
endif()
...
# --test fails input that is not gzip from stdin as well, where nothing may pass through.
execute_process(COMMAND ${EXE} -t INPUT_FILE ${WORKDIR}/data.orig RESULT_VARIABLE rc ERROR_QUIET)
if(rc EQUAL 0)
    message(FATAL_ERROR "--test passed plain input on stdin")
endif()
