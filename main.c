...
static int32_t run_stream(FILE *in, FILE *out, const gzng_options *opt, uint32_t mtime, const char *name,
    ...
    if (opt->decompress)
        return gzng_decompress_stream(in, out, NULL, 0, opt->threads, total_in, total_out);
    if (opt->transparent) {
        uint64_t n = 0;
...
static int32_t process_stdio(const gzng_options *opt) {
    ...
    if (opt->test_mode) {
        ...
        if (test_peek(stdin, "stdin", opt, head, &head_len) != 0)
            return GZ_ERROR;
        rc = gzng_decompress_stream(stdin, NULL, head, head_len, opt->threads, &total_in, &total_out);
    } else {
...
static int32_t test_file(FILE *in, const char *path, const gzng_options *opt) {
    ...
    if (test_peek(in, path, opt, head, &head_len) != 0)
        return GZ_ERROR;
    if (gzng_decompress_stream(in, NULL, head, head_len, opt->threads, &total_in, &total_out) != 0)
        return GZ_ERROR;
    if (opt->verbose && !opt->quiet)
        fprintf(stderr, "%s:\t OK\n", path);
    return GZ_OK;
